...
writerloss=SummaryWriter("runs/exp-1")
        ...
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if ((epoch + 1) % self.arg.eval_interval == 0) or (
                    ...
                    self.test()  #rs.append(self.test())
                    writerloss.add_scalar('test/loss',self.epoch_info['mean_loss'],
                                          self.meta_info['epoch']+1)
                    self.io.print_log('Done.')

                if (epoch + 1 == self.arg.num_epoch):
                    self.io.print_log('Max Top Epoch: {}'.format(self.max_epoch))
                    self.io.print_log('Max Top1: {}'.format(self.max_top1))
                    self.io.print_log('Max Top5: {}'.format(self.max_top5))

            endtime = datetime.datetime.now()
            usedtime = endtime - starttime
            totalsecond = usedtime.total_seconds()
            print('time',totalsecond)
            #rs=np.array(rs)
            #rs_best_index=np.argmax(rs,axis=0)
            #print('best accuracy:', rs_best_index[0], rs[rs_best_index[0]][0], rs[rs_best_index[0]][1])
        # test phase
